...
public class GamePlay implements GUIdisplayable {
	...
	private void npcInteractWithPlayer() {
		for (NPC npc : allNPC) {
			if (player.getRoom().equals(npc.getCurrentRoom())) {
				if ((int) (Math.random() * 100) + 1 > 60) {
					...
					npc.setTeleportedPlayerLastEncounter(true);
				} else if ((int) (Math.random() * 100) + 1 > 60) {
					player.subtractTime(50);
					npc.setSubtractedTimeFromPlayerLastEncounter(true);
				}
			}
		}
	}
	 ...
	 *
	 * @param item is the Item from the use method
	 * @return a boolean based on wether or not the key was used
	 */
	 ...
	 *
	 * @param item is the item from the use method
	 * @return a boolean based on wether or not the the item was used succsesfully
	 */
	...
	/**
	 * @return an ObservableList containing scores
	 */
	@Override
	...
	/**
	 * @return an ArrayList containing NPC's
	 */
	@Override
	...
	/**
	 * @return an ArrayList containing Boss objects
	 */
	@Override
	...
	/**
	 * The isCodeCorrect method is used to check, if the code entered when a room
	 * with the boolean escapeRoom is true, is the correct code
	 *
	 * @param code is the code that is entered
	 * @return a boolean based on wether or not the code is correct
	 */
	@Override
	...
	/**
	 * The getListOfFile is used to get all files from a folder
	 *
	 * @return an ObservableList with the files in the folder "GameFiles" in the
	 * project folder.
	 */
	@Override
